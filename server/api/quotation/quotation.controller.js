...
function createQ(quot, filename, customer) {
  ...
  if (mode == 'development') {
    ...
    var path = './dist/client/assets/quotation/' + filename + '.pdf';
  }
...
export function getAllQuotes(req, res) {
  ...
  return Quotation.findAll({offset: offset, limit: limit, order: [['_id', 'DESC']], include: [{model: User, as: 'distributor'}, {model: User, as: 'customer'}]})
  .then(respondWithResult(res))
  .catch(handleError(res));
}

export function getAllQuoteCount(req, res) {
  return Quotation.count({include: [{model: User, as: 'distributor'}, {model: User, as: 'customer'}]})
  .then(respondWithResult(res))
  .catch(handleError(res));
}
